  ...
  const createApplication = async (req, res) => {
    try {
      ...
      const jobTitle = await Job.findById(jobId).then(job => job.jobTitle).catch(err => {
        console.error('Error fetching job title:', err);
        return '';
      });

      console.log("Creating application for jobId:", jobId, "by applicantId:", applicantId, "with jobTitle:", jobTitle);
  ...
  const getApplicationByApplicantId = async (req, res) => {
    try {
      ...
      let applications = await Application.find({ applicantId })
        .populate('recruiterId', 'companyName').populate('jobId', 'jobTitle')
        .lean();
        
      // Manually attach Job details to ensure frontend parser gets jobTitle
